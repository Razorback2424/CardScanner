...
completed 2026-09-19**, and **Slice F validation-only and one-off revision-1
authority rehearsal completed 2026-09-19**. Live provider/candidate,
physical-device/offline, and production cutover gates remain open.
...
**Current sequencing decision (2026-09-19):** keep the hosted production
baseline at signed revision 1 and keep the committed app configuration at
`bundled-validation-only`. The online validation-only rehearsal, no-store-
mutation/relaunch evidence, and the focused/broader catalog suites are green
after pushed commit `54960fd`. A deliberately one-off `remote-authority` build
also proved revision-1 activation, persistence, and same-revision idempotency
on the dedicated simulator. The revision-2 no-op candidate generated by
workflow run [35443012617](https://github.com/Razorback2424/CardScanner/actions/runs/35443012617)
matched revision 1 at the descriptor, count, fingerprint, and snapshot levels,
but was canceled before signing or deployment. The first actual catalog change
should therefore become revision 2; do not create an artificial production
revision solely for lifecycle testing.

The next meaningful gate is a physical-device remote-authority rehearsal,
including the deferred offline case. Only after that evidence should the
committed production mode change or a production build be shipped. Keep the
separate staging project, site, DNS name, and deployment job deferred until a
distinct environment materially reduces rollout risk.
...
key/pin are verified, but this plan does not claim remote-authority production
readiness or provider/device certification.
...
one-off revision-1 authority simulator gates are complete. The remaining
authority gate is physical-device evidence, including real connectivity loss;
the committed production mode remains `bundled-validation-only` until that
gate and the release checks pass. Staging remains deferred.
...
**Slice F implementation notes (2026-09-19):** The app now defaults to
...
explicit `remote-authority` mode is available only for a deliberately one-off
rehearsal build; the committed production xcconfig remains validation-only.
The production-host validation-only run and the focused/broader suites passed
after `54960fd`; the one-off simulator authority run activated hosted revision
1, persisted it, and handled the identical relaunch envelope as `notModified`.
