...
| C-09 | The release framework and CloudKit audit describe storage/sync continuity as source-hardened with only **external enrollment** outstanding, which reads as “no known code defect in this area.” | [`../audits/defect_review_pass_2.md`](../audits/defect_review_pass_2.md) F01/F02 are deterministic source traces in `CollectionStorageBootstrap`/`CloudRestorationReadiness` that do not depend on enrollment: the production readiness default strands the clean-install flow, and the headless preflight builds a CloudKit container for a non-`.attached` manifest. | **Open — not resolved.** The enrollment gate and these two defects are independent. Treat “source hardening exists” as scoped to the reviewed policy logic, not to the production dependency wiring. Fix tracked in the pass-2 audit; gate rows updated in the release framework §14. |
| C-10 | C-05 records the scanner workflow F1–F5 as resolved, which reads as “this area has no open findings.” | Pass 2 F03 is a **new suspected** finding at `ScannerViewModel.swift:2271` (`beginPendingResolution` can discard a choose-handler operation), not a restatement of F1–F5. The C-05 lifecycle fixes were separately re-verified and remain correct. | **Both true.** C-05 stays resolved for its own scope. F03 is a distinct, lower-confidence finding recorded in the pass-2 audit; it does not reopen C-05. |
| C-11 | `progress.md` and the entries above characterise the full-suite failures in aggregate as “unrelated fixture/source-environment or signal-kill failures.” | The pass-2 run at `a4375df` executed 1,277 with 6 skipped and 40 failures; per-suite triage found 36 fixture/environment, 1 load-sensitive flake, and **3 substantive** (`OwnershipLedgerCompletenessTests`, pass-2 F04/F05, each reproduced in isolation). The earlier 1,256-discovered/48-failure figure was recorded at `0b4ac34` and counts discovered rather than executed tests. | **Partially incorrect.** The aggregate “unrelated” wording must not be reused. Record per-suite triage, not a single classified count, and convert corpus-dependent tests to skips (pass-2 F06, [`release_followups.md`](release_followups.md) RF-6) so the exit status can carry evidence again. |
...
| C-13 | [`browse_screen_spec.md`](browse_screen_spec.md) § 8 specified the set tile footer as `3 of 207` — a distinct-collector-number numerator over `set.cardCount` with the unit `cards` — while the set screen showed master-set **variation** slots owned over the built checklist length. | The current source now treats an all-zero provider variation breakdown as unpublished, carries standard/expanded checklist slot counts in the manifest, and uses `CatalogSetCompletionIndex` for both the tile and set screen. The spec subsection was reconciled on 2026-09-15. | **Resolved in source, contract, and deterministic evidence.** Focused Browse selectors pass 128/128 with 0 failures, and the external-SSD snapshot check confirms denominator parity for all 157 entries. Pokémon uses the checklist-backed `variations` unit, with the bounded overflow explicitly labeled `cards`; Magic retains provider card totals. Visual confirmation is still intentionally open. |
