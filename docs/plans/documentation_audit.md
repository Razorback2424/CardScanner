...
| C-13 | [`browse_screen_spec.md`](browse_screen_spec.md) § 8 specifies the set tile footer as `3 of 207` — a distinct-collector-number numerator over `set.cardCount` with the unit `cards` — while the set screen shows master-set **variation** slots owned over the built checklist length. | Measured against the bundled snapshot at `a4375df`: `entry.set.cardCount` differs from the standard-tier slot count for **134 of 160 entries**, and is `0` for **45** because `PokemonMasterSetDefinition.masterCount` ([`BrowseCatalog.swift:1061`](../../TradingCardScanner/Services/BrowseCatalog.swift:1061)) reads TCGdex's published zeros as a published breakdown. The two surfaces therefore cannot agree. | **Open — not resolved.** Slice B of [`browse_set_directory_remediation_plan.md`](browse_set_directory_remediation_plan.md) makes the built checklist the single denominator for both surfaces and changes the Pokémon unit to `variations`. § 8's `n of m` shape, `n owned` fallback, and hide-when-zero rule are unchanged and remain the acceptance contract. Update § 8 when Slice B lands; do not treat the spec's `cards` wording as current for Pokémon in the meantime. |
